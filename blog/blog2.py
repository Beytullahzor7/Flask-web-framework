...
class RegisterForm(Form):
    ...
    email = StringField("Email Adresi", validators=[validators.Email(message="Lütfen Gecerli Bir email Adresi Giriniz...")])
...
@app.route("/")
def index():
    return render_template("index157.html")
...
def login():
    ...
    if request.method == "POST":
        ...
        result = cursor.execute(sorgu,(username,)) #demet halinde olması için sona , koyduk
        ...
        if result > 0:
            ...
            if sha256_crypt.verify(password_entered, real_password):
                flash("Basarıyla Giriş Yaptınız...","success")
...
def delete(id):
    ...
    if result > 0:
        ...
        flash("Böyle bir makale yok veya bunu silmeye yetkiniz yok!","danger")
        return redirect(url_for("index"))
...
def update(id):
    if request.method == "GET":
        ...
        flash("Makale Basarı İle Güncellendi","success")
        return redirect(url_for("dashboard"))
